...
{
    return (ct1.ascii == ct2.ascii && isColor(ct1.color.fg,ct2.color.fg) && isColor(ct1.color.bg,ct2.color.bg));
}
...
void printItemList(IdentificationData *deviceIdData, item_container *itc, int current_selection, bool disp_name, bool autoIdentifiedItems)
{
    ...
    for (int i = 0; i < page_size; ++i)
    {
        ...
        item_t itm = itc->getItem(index)->getType();
        ...
        if (ic != ITEM_DEVICE) 
        {
            addChar(point(2, i + 1), symbol_item[(int)itm]);
        }
        else 
        {
            int deviceSymbolIndex = deviceIdData->identificationMap[(int)itm - DEVICE_OFFSET];

            if (!autoIdentifiedItems || deviceIdData->discoveryMap[(int)itm - DEVICE_OFFSET]) 
            {
                addChar(point(2, i + 1), deviceSymbols[deviceSymbolIndex]);
            }
            else 
            {
                addChar(point(2, i + 1), symbol_item[(int)itm]);
        ...
        if (ic == ITEM_DEVICE)
        {
            displayDeviceItemStats(deviceIdData, itc->getItem(index),print_loc,autoIdentifiedItems);
        }
...
void displayDeviceItemStats(IdentificationData *deviceIdData, item *it, point loc, bool autoIdentifiedItem)
{
    int index = (int)it->getType() - DEVICE_OFFSET;
    STRING device_name = (deviceIdData->discoveryMap[index] || autoIdentifiedItem) ? it->getStats()->name + " " : unidentifiedDeviceNames[deviceIdData->identificationMap[index]];
    STRING charge_counter = (it->getChargeKnown() || autoIdentifiedItem) ? "(" + sint2String(it->getStats()->misc_val) + ")" : "";
    addGFXString(colorpair_gray_black,device_name + charge_counter,loc.x(),loc.y());
}

void displayArmorStats(item *it, point loc)
{
    STRING armor_name = it->getStats()->name;
    int dv = it->getStats()->armstats.basedv_value;
    int pv = it->getStats()->armstats.basepv_value;
    int mv = it->getStats()->armstats.metallic_bonus;
    int ev = it->getStats()->armstats.energy_bonus;
    STRING values_indicator = ((dv != 0) ? " DV:" + sint2String(it->getStats()->armstats.basedv_value) : "") +
                              ((pv != 0) ? " AC:" + sint2String(it->getStats()->armstats.basepv_value) : "") +
                              ((mv != 0) ? " PJ:" + sint2String(it->getStats()->armstats.metallic_bonus) : "") +
                              ((ev != 0) ? " EN:" + sint2String(it->getStats()->armstats.energy_bonus) : "");
...
void mapdisplay::printCell(map *m, point p, chtype ct)
{
    ...
    else if (!m->getm(p) && m->getFire(p) != CURSOR_GEN)
    {
        added = blank_ch;
    }
