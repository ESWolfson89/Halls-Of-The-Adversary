...
void Game::displayMappedOutLevel()
{
    ...
    {
        if ((terrainAt(point(i, j)) != WALL_INDESTRUCT && terrainAt(point(i, j)) != WALL_DESTRUCT) ||
...
{
    followingAllies.resize(0);

    for (auto i = 0; i < level_obj.getNumNPCs(); i++)
    {
        if (getBeing(i)->getAllyValue() >= 2)
        {
            followingAllies.push_back(*getBeing(i));
        }
    }

    for (auto i = 0; i < (int)followingAllies.size(); i++)
    {
        level_obj.destroyNPC(followingAllies[i].at());
    }
}

void Game::addStartingAllies()
{
    point p;

    point e = level_obj.getEntrance();

    being_stats stats = procgen_starthumanally[0];

    do
    {
        p = level_obj.getRandWalkablePoint(e.x()-4,e.y()-4,e.x()+4,e.y()+4);
    } while (isAt(p,e));

    level_obj.addBeing(p, stats.max_hit_points, stats, false, true);
    getBeingAt(p)->setFactoryAscii((int)'@');
    getBeingAt(p)->setFactoryColor(color_darkgreen);
    getBeingAt(p)->setAllyValue(2);
}

void Game::deallocateGameData()
{
    followingAllies.resize(0);
    for (auto i = 0; i < NUM_REGULAR_LEVELS; i++)
    {
        deallocateGenericData(&mainLevels[i]);
    }
    mainLevels.resize(0);
    deallocateGenericData(&level_obj);

    clearInventory(&player);

    const item_stats nullStats = item_default_stats[(int)NIL_i];
    const being_stats playerStats = being_default_stats[(int)PLAYER];

    setWeapon(&player, nullStats);
    setArmor(&player, nullStats);

    player.setBeing(point(1,1), playerStats.max_hit_points, playerStats, false, true, 0, "player");
}
...
void Game::initGame()
{
    ...
    addToInventory(&player, item_default_stats[(int)PISTOL_10MM],1);
    addToInventory(&player, item_default_stats[(int)HANDGUN_AMMO_GEN],24);
    setWeapon(&player, 0);

    playerReloadFirearm();

    setCredits(&player, 0ULL);

    num_game_turns = 0;

    last_regular_level = 1;
    level_number = 1;

    for (auto i = 0; i < NUM_REGULAR_LEVELS; i++)
    {
        level l;
        l.initLevel(i+1, LEV_REGULAR);
        if (i == 0)
        {
            l.initLevel1SpecialRoom();
        }
        mainLevels.push_back(l);
    }

    level_obj = mainLevels[0];

    level_obj.setSidewaysElevatorSpawnFlag(false);
    level_obj.setBlockedArchwaySpawnFlag(false);
...
void Game::initItemMaps() 
{
    deviceIdData.discoveryMap.clear();
    deviceIdData.identificationMap.clear();

    VECTOR<int> mapIndices;

    for (auto i = 0; i < NUM_TOTAL_DEVICES; i++) 
    {
        deviceIdData.discoveryMap[i] = false;
        mapIndices.push_back(i);
    }

    RANDOM_SHUFFLE(mapIndices.begin(), mapIndices.end());

    for (auto i = 0; i < NUM_TOTAL_DEVICES; i++) 
    {
        deviceIdData.identificationMap[i] = mapIndices[i];
    }
}
...
void Game::gotoPrevLevel()
{
    ...
    if (terrainAt(playerAt()) == PREV_STAIRS) 
    {
        int e_lev = level_obj.getElevator(playerAt())->getLevel();
    ...
    else if (terrainAt(playerAt()) == SIDEWAYS_ELEVATOR) 
    {
        msgeAdd("Press '>' to take the sideways elevator.",colorpair_gray_black);
    } 
    else if (terrainAt(playerAt()) == BLOCKED_ARCHWAY) 
    {
...
void Game::gotoNextLevel()
{
    ...
    if (terrainAt(playerAt()) == NEXT_STAIRS) 
    {
        int e_lev = level_obj.getElevator(playerAt())->getLevel();
...
void Game::promptUseSidewaysElevator()
{
    ...
    uint credit_cost = 100 + (uint64)ROLL(1000) + 5000ULL*(level_number - 1) + (uint64)ROLL(300000)*(level_number - 1);
    switch (level_obj.getSpecLevVal()) 
    {
        case((int)LEV_REGULAR - 1):
        {
            ...
            if (ibuf.checkForKey('y'))
            {
                if (credits(&player) >= credit_cost)
                {
                    decCredits(&player, credit_cost);
                    mbuf.clearAndDeleteAllMessages();
                    printPlayerStats();
...
void Game::changeLevel(int e_lev, lev_type lt)
{
    ...
    deallocateGenericData(&mainLevels[old_level_number - 1]);
    mainLevels[old_level_number - 1] = level_obj;

    deallocateGenericData(&level_obj);
    level_obj = mainLevels[level_number - 1];
    ...
    player_quick_fire_spot = playerAt();
}
...
void Game::setupAllies() 
{
    ...
    for (auto i = 0; i < (int)followingAllies.size(); ++i)
    {
            ...
            while(!end_condition) 
            {
                for (auto x = -dist_iter; x <= dist_iter; ++x) 
                for (auto y = -dist_iter; y <= dist_iter; ++y) 
                {
                    p.set(e.x()+x,e.y()+y);

                    if (inRange(p, 1, 1, MAPWID - 1, MAPHGT - 1)) 
                    {
                        if (!isAt(e, p) && NPCAt(p) == NIL_n && !end_condition)
                        {
            ...
            followingAllies[i].moveTo(bp);
            level_obj.addBeing(followingAllies[i]);

            setNPCAt(bp, followingAllies[i].getType());
            getBeingAt(bp)->setID(level_obj.getNPCIDCounter());
            level_obj.incNPCIDCounter();
            getBeingAt(bp)->setAttackID(-1);
            getBeingAt(bp)->setFleeID(-1);
            getBeingAt(bp)->setActivation(true);
    }
}
...
void Game::calculatePlayerLOS()
{
    ...
    int d = beingStats(&player)->detect_distance;

    getMap()->setAllCellsUnvisible();

    // for all valid cells in range of player
    for (auto x = playerAt().x() - d; x <= playerAt().x() + d; ++x)
    for (auto y = playerAt().y() - d; y <= playerAt().y() + d; ++y) 
    {
        p.set(x, y);
        ...
        if (!linetracer.isBlocking(getMap(), playerAt(), p, true, false, false)) 
        {
            getMap()->setm(p, true);
            getMap()->setv(p, true);
...
void Game::checkForBeingStateEvent(being* provoker, being* target, bool fire_weapon)
{
    ...
    {
        player.setAttackID(target->getID());
    }
}

void Game::melee(being *attacker, being *target)
{
    int numStartNPCS = level_obj.getNumNPCs();

    checkForBeingStateEvent(attacker,target,false);

    for (auto i = 0; i < beingStats(attacker)->melee_per_turn; ++i) 
    {
        if (numStartNPCS != level_obj.getNumNPCs()) 
        {
            return;
        }
        ...
        if (baseHitChance(attacker,target,false)) 
        {
            ...
            totalDamage = baseMeleeDamage(attacker);

            int critMult = checkForCriticalHit(weapon(attacker), true);

            totalDamage *= critMult;

            totalDamage -= getArmorProtectionValue(attacker,target,true,false,false);
            ...
            {
                checkForStunEvent(attacker, target, totalDamage, true);
            }

            checkForDrainLifeEvent(attacker,target);

            if (getMap()->getv(target->at())) 
            {
                damageMsgeAdd(sint2String(totalDamage), weapon(attacker)->getType(), getExactSymbolFromNPC(attacker),
                    getExactSymbolFromNPC(target), (totalDamage >= target->getHP()), false, getMap()->getv(attacker->at()), true, critMult);

            }

            damageBeing(target, target->at(), totalDamage);
        } 
        else 
        {
...
void Game::resetBeingAttackAndFleeIDs(int id)
{
    ...
    if (player.getAttackID() == id) 
    {
        player.setAttackID(-1);
    }

    if (player.getFleeID() == id) 
    {
        player.setFleeID(-1);
    }
    ...
    for (auto i = 0; i < level_obj.getNumNPCs(); ++i) 
    {
        ...
        if (getBeing(i)->getAttackID() == id) 
        {
            resetFlag = true;
            getBeing(i)->setAttackID(-1);
        }

        if (getBeing(i)->getFleeID() == id) 
        {
            resetFlag = true;
            getBeing(i)->setFleeID(-1);
        }
...
{
    if (!b->isDead() && dam >= b->getHP()) 
    {
        if (b->getStats()->explodesUponDeath)
        {
            createDeathExplosion(b, p);
        }
        b->killBeing();
    } 
    else 
...
void Game::createDeathExplosion(being* b, point p)
{
    if (getMap()->getv(p))
    {
        msgeAdd(getNameToPrint(b, true, true) + " explodes!", colorpair_orange_black);
    }

    createExplosion(b, getDeathExplosionDamage(b), p, false);
}

void Game::playerEquipWeapon(STRING name, int selection)
{
    setWeapon(&player, selection);
    msgeAdd("You are equip the " + name + ".", colorpair_white_black);
}

void Game::playerWearArmor(STRING name, int selection)
{
    setArmor(&player, selection);
    msgeAdd("You put on the " + name + ".", colorpair_white_black);
}

bool Game::checkPlayerUseItem(int selection)
{
    item_stats * istats = playerInventory()->getItem(selection)->getStats();
    ...
    switch(istats->category) 
    {
        ...
        {
            upgradeEquippedWeapon(&player, selection);
            return false;
        }
        case(ITEM_STATBOOSTER):
        {
            upgradeAttributes(&player, selection);
            return false;
        }
        case(ITEM_MEDKIT):
        {
            useMedkit(&player, selection);
            return false;
        }
        case(ITEM_DEVICE):
        {
...
void Game::playerUseDrill(int selection)
{
    ...
    point loc = addPoints(playerAt(), ibuf.getDelta());

    if (terrainAt(loc) == WALL_INDESTRUCT) 
    {
        setTerrainAt(loc,FLR_GEN);
        decrementDeviceCounter(&player, selection);
...
void Game::playerUseTeleport(int selection)
{
    ...
    msgeAdd("Use the directional keys to choose a teleport destination.",colorpair_white_black);
    decrementDeviceCounter(&player, selection);
    moveCursor(true);
    player_quick_fire_spot = playerAt();
}
...
void Game::playerUseTimeStopper(int selection)
{
    ...
    setNPCAIActivation(false,playerInventory()->getItem(selection)->getStats()->misc_val + 1);
    playerInventory()->removeItem(selection, 1);
}

void Game::playerUseCorroder(int selection)
{
    int corrosiveness = playerInventory()->getItem(selection)->getStats()->misc_val;

    if (playerArmor()->getType() == NIL_i) 
    {
        msgeAdd("You are splashed with corrosive slime! It burns!", colorpair_red_black);
        damagePlayer(getPlayer(), ROLL(corrosiveness * 5) + 3);
    } 
    else 
    {
        msgeAdd("You are splashed with corrosive slime! Your armor corrodes!", colorpair_red_black);

        item_stats istats = *playerArmor()->getStats();
        armor_stats astats = istats.armstats;

        int newMetallicBonus = astats.metallic_bonus - (ROLL(corrosiveness /2) + 1);
        int newEnergyBonus = astats.energy_bonus - (ROLL(corrosiveness /2) + 1);
        int newPVBonus = astats.basepv_value - (ROLL(corrosiveness) + 1);

        if (newMetallicBonus < 0) 
        {
            newMetallicBonus = 0;
        }
        ...
        astats.metallic_bonus = newMetallicBonus;
        astats.energy_bonus = newEnergyBonus;
        astats.basepv_value = newPVBonus;
        istats.armstats = astats;
        playerArmor()->setStats(istats);
    }
    playerInventory()->removeItem(selection, 1);
}
...
{
    for (auto i = 0; i < level_obj.getNumItemPiles(); ++i)
    {
        getMap()->setv(level_obj.getPile(i)->getLoc(),true);
        getMap()->setu(level_obj.getPile(i)->getLoc(),blank_ch);
    }

    for (auto x = 0; x < MAPWID; ++x)
    for (auto y = 0; y < MAPHGT; ++y)
    {
        item_t itemValue = getMap()->getItem(point(x, y));

        if (itemValue != NIL_i)
        {
            if (item_default_stats[(int)itemValue].category != ITEM_DEVICE)
            {
                disp.printCell(getMap(), point(x, y), symbol_item[(int)itemValue]);
            }
            else
            {
                int index = deviceIdData.identificationMap[(int)itemValue - DEVICE_OFFSET];
                disp.printCell(getMap(), point(x, y), deviceSymbols[index]);
            }
        }
    }

    printCustomItemSymbols();
...
void Game::displayAllNPCs()
{
    for (auto i = 0; i < level_obj.getNumNPCs(); ++i)
    {
        getMap()->setu(beingAt(i), blank_ch);
        getMap()->setv(beingAt(i), true);
    }

    for (auto x = 0; x < MAPWID; ++x)
    for (auto y = 0; y < MAPHGT; ++y)
    {
        NPC_t beingValue = getMap()->getNPC(point(x, y));

        if (beingValue != NIL_n)
        {
            disp.printCell(getMap(), point(x, y), symbol_NPC[(int)beingValue]);
        }
    }

    printCustomNPCSymbols();

    msgeAddPromptSpace("Level population: " + sint2String(level_obj.getNumNPCs()),colorpair_white_black);

    for (auto i = 0; i < level_obj.getNumNPCs(); ++i)
    {
        getMap()->setv(beingAt(i), false);
    }
}

void Game::useDevice(being *b, int selection) {
    item_t it = b->getInv()->getItem(selection)->getType();
    deviceIdData.discoveryMap[(int)it - DEVICE_OFFSET] = true;
    b->getInv()->getItem(selection)->setChargeKnown(true);
    ...
    switch(it) 
    {
        ...
        case(CORRODING_DEVICE):
        {
            playerUseCorroder(selection);
            break;
        }
        default:
        {
...
void Game::moveCursor(bool isTeleporting)
{
    point cursorLoc = playerAt();
    point tempLoc = playerAt();

    do 
    {
        disp.setAndPrintFire(getMap(),cursorLoc,CURSOR_GEN);
        ...
        if (ibuf.getAction() == INP_DELTA) 
        {
            tempLoc = addPoints(cursorLoc,ibuf.getDelta());
            if (inRange(tempLoc,1,1,MAPWID-2,MAPHGT-2)) 
            {
                disp.setAndPrintFire(getMap(),cursorLoc,NIL_f);
                cursorLoc = tempLoc;
            }
        }
    ...
    if (isTeleporting)
    {
        if (!unWalkableCell(getMap()->getCell(cursorLoc))) 
        {
            changeBeingTile(playerAt(), cursorLoc,PLAYER);
            getPlayer()->moveTo(cursorLoc);
...
void Game::addHitMissIndicatorMessage(being * attacker, being * target, bool hit_success)
{
    ...
    if (pointDist(attacker->at(),target->at()) > 1) 
    {
        distanceString = " from a distance";
        end_string = "!";
    }
    ...
    if (attacker->getType() != PLAYER && target->getType() != PLAYER) 
    {
        if (getMap()->getv(attacker->at()) || getMap()->getv(target->at())) 
        {
            msgeAdd(getNameToPrint(attacker, true, true) + " " + NPC_string + " " + getNameToPrint(target, true, false) +
                distanceString + end_string, colorpair_white_black);
            return;
        }
    }
    ...
    {
        msgeAdd("You " + p_string + " " + getNameToPrint(target, true, false) + distanceString + end_string, colorpair_white_black);
    }

    if (target->getType() == PLAYER) 
    {
        msgeAdd(getNameToPrint(attacker, true, true) + " " + NPC_string + " you" + distanceString + end_string, colorpair_white_black);;
    }
}
...
void Game::deallocateGenericData(level *l)
{
    l->deleteAllPiles();
    ...
    {
        l->destroyAllNonAllies();
    }

    l->destroyAllDoors();
    l->destroyAllElevators();
    l->destroyAllMachines();
    l->destroyAllSatellites();
    mbuf.clearAndDeleteAllMessages();
...
{
    if (getPlayer()->isDead() && getPlayer()->getHP() > 0) 
    {
        getPlayer()->setHP(0);
        printPlayerStats();
...
void Game::checkNPCDefeatedEvents() {
    ...
    for (auto i = 0; i < level_obj.getNumNPCs(); ++i) 
    {
        ...
        if (level_obj.getBeing(i)->isDead()) 
        {
            ...
            {
                if (!beingStats(getBeing(i))->explodesUponDeath)
                {
            ...
            bool dropEverything = (level_obj.getBeing(i)->getType() == ARMSDEALER_NPC ||
            ...
            beingDropAllItems(level_obj.getBeing(i),level_obj.getBeing(i)->at(), dropEverything);
...
// util
void Game::checkAttributeChange(being * b, int increment) {
    switch (ROLL(2)) 
    {
        case(0):
        {
            b->incAttributes(increment, 0, 0);
            break;
        }
        case(1):
        {
            b->incAttributes(0, increment, 0);
            break;
        }
        case(2):
        {
            b->incAttributes(0, 0, increment);
            break;
        }
    }
}
...
void Game::checkExperience(being *b, uint e)
{
    ...
    for (i = init_level; i < MAX_EXP_LEVEL; ++i) 
    {
...
void Game::checkForDrainLifeEvent(being* attacker, being* target) 
{
    if (target->getAllyValue() > 0) 
    {
        if (PERCCHANCE() <= attacker->getStats()->drain_experience_chance) 
        {
            int expLevel = target->getLevel();

            if (expLevel > 1) 
            {
                target->setMaxHP(target->getStats()->max_hit_points - ROLL(target->getLevel()) - 1);
                target->setHP(target->getStats()->max_hit_points);
                target->setExperience(exp_levelup_values[target->getLevel() - 2] - 1);
                target->decLevel(1);
                target->decHealing();
                checkAttributeChange(target, -1);

                if (target->getType() == PLAYER)
                {
                    reDisplayRightScreenSide();
                    msgeAdd("Your life force is drained! You went down to level " + sint2String(target->getLevel()) + ".", colorpair_purple_black);
                } 
                else if (getMap()->getv(target->at())) 
                {
                    msgeAdd(getNameToPrint(target, true, true) + "'s life force is drained! " +
                        getNameToPrint(target, true, true) + " went down to level " + sint2String(target->getLevel()) + ".", colorpair_purple_black);
                }
            }
        }
    }
...
void Game::chatWithNPC()
{
    ...
    if (ibuf.getAction() == INP_DELTA) 
    {
        ...
        if (adj_npc != NIL_n) 
        {
            ...
            else if (adj_npc != ARMSDEALER_NPC) 
            {
                NPCCommunicate(b, adj_npc, true);
            } 
            else if (isAt(b->at(), b->getInitLoc())) 
            {
                buyFromArmsDealer(b);
            } 
            else 
...
void Game::enterDebugMode()
{
    ...
    if (!debug_mode)
    {
        ...
        if (ibuf.checkForKey('y')) 
        {
            ...
            playerInventory()->addToContainer(item_default_stats[(int)DEBUG_ARMOR],1);

            for (auto i = (int)METAL_ROD; i <= (int)SWORD_GIANT; ++i) 
            {
                playerInventory()->addToContainer(item_default_stats[i], 1);
            }

            for (auto i = (int)PISTOL_10MM; i <= (int)PULSE_CANNON; ++i) 
            {
                playerInventory()->addToContainer(item_default_stats[i], 1);
            }

            for (auto i = (int)HANDGUN_AMMO_GEN; i <= (int)PLASMA_AMMO; ++i) 
            {
                playerInventory()->addToContainer(item_default_stats[i], 100);
            }

            for (auto i = 0; i < 40; ++i) 
            {
                playerInventory()->addToContainer(getModifiedUpgradeCharacteristics(DOUBLEBARRELLED_SHOTGUN_UPGRADE), 1);
            }

            for (auto i = (int)HEALTH_BOOSTER; i <= (int)GIANT_MEDKIT; ++i) 
            {
                playerInventory()->addToContainer(item_default_stats[i], 3);
            }

            for (auto i = 0; i < 5; ++i) 
            {
                playerInventory()->addToContainer(getModifiedDeviceCharacteristics(DRILL_DEVICE, ROLL(10) + 1), 1);
            }

            for (auto i = 0; i < 5; ++i) 
            {
                playerInventory()->addToContainer(getModifiedDeviceCharacteristics(ITEMDETECTOR_DEVICE, ROLL(10) + 1), 1);
            }

            for (auto i = 0; i < 5; ++i) 
            {
                playerInventory()->addToContainer(getModifiedDeviceCharacteristics(MAPPING_DEVICE, ROLL(10) + 1), 1);
            }
                
            for (auto i = 0; i < 5; ++i) 
            {
                playerInventory()->addToContainer(getModifiedDeviceCharacteristics(NPCDETECTOR_DEVICE, ROLL(10) + 1), 1);
            }

            for (auto i = 0; i < 5; ++i) 
            {
                playerInventory()->addToContainer(getModifiedDeviceCharacteristics(TELEPORT_DEVICE, ROLL(10) + 1), 1);
            }

            for (auto i = 0; i < 5; ++i) 
            {
                playerInventory()->addToContainer(getModifiedDeviceCharacteristics(TIMESTOPPER_DEVICE, ROLL(10) + 1), 1);
            }

            for (auto i = 0; i < 5; ++i) 
            {
                playerInventory()->addToContainer(getModifiedDeviceCharacteristics(CORRODING_DEVICE, ROLL(4) + 1), 1);
            }

            getPlayer()->incAttributes(70,70,70);
            incCredits(&player, 2000000000ULL);
        }
    }
}
    ...
    if (b->getAllyValue() > 0)
    {
        if (b->getAttackID() == -1) 
        {
            ...
            if (b->getAttackID() == -1) 
            {
                ...
                for (auto i = 0; i < level_obj.getNumNPCs(); ++i) 
                {
                    if (getBeing(i)->getAllyValue() == 2 && getBeing(i)->getID() != b->getID())
                    {
                        last_ally_loc = beingAt(i);
                        if (!isAt(pfinder.dijkstra(getMap(), b->at(), last_ally_loc), b->at())) 
                        {
                            b->setPathfindDest(last_ally_loc);
                            return;
...
bool Game::checkNPCFightEvent(being *b)
{
    ...
    int distance = pointDist(b->at(), a->at());

    if(distance <= b->getStats()->melee_distance) 
    {
        bool pathBlocked = false;
        if (distance > 1)
        {
            pathBlocked = (linetracer.isBlocking(getMap(), b->at(), a->at(), true, false, true) ||
                (a->getType() == PLAYER && !getMap()->getv(b->at())));
        }
        if (!pathBlocked) 
        {
...
void Game::checkNPCMiscEvent(being *b)
{
    ...
    {
        if (PERCCHANCE() <= b->getStats()->aggressiveness)
        {
            if (pointDist(b->at(), getPlayer()->at()) <= b->getStats()->detect_distance)
            {
                b->setAttackID(0);
            }
        }
...
{
    uint64 fee = (uint64)(ROLL(5000)*level_number*level_number*level_number + 250);
...
{
    if (b->getType() == ARMSDEALER_NPC || b->getType() == ELITEGUARD_NPC) 
    {
        if (b->getAttackID() == -1 && b->getFleeID() == -1) 
        {
...
bool Game::checkNPCSummonEvent(being *b)
{
    ...
    if (b->getAllyValue() == 2 || b->getStats()->can_summon == false || ROLL(449 - b->getStats()->danger_level) > 0)
    {
        return false;
    }
...
void Game::activateOneNPC(int n)
{
    ...
    if (pointDist(b->at(), b->getPathfindDest()) <= b->getStats()->detect_distance || b->getFleeID() > -1)
    {
...
bool Game::checkNPCFireNonWeaponProjectile(being* b)
{
    if (PERCCHANCE() <= beingStats(b)->spitAcidChance)
    {
    }
    return false;
}
...
{
    if (!game_active || !player_has_moved || getPlayer()->isDead())
    {
        return;
    }
...
{
    addGFXString(colorpair_gray_black,"                                        ",0,SHOWHGT);
    addGFXString(colorpair_gray_black,"C:" + int2String((uint64)getPlayer()->getCredits()),0,SHOWHGT);
    addGFXString(colorpair_gray_black,"HIT:" + sint2String(getPlayer()->getHP()) + "/" + sint2String(getPlayer()->getStats()->max_hit_points),14,SHOWHGT);
    addGFXString(colorpair_gray_black," S:" + int2String((uint64)getPlayer()->getStats()->strength) +
                 " D:" + int2String((uint64)getPlayer()->getStats()->dexterity) +
                 " A:" + int2String((uint64)getPlayer()->getStats()->accuracy),29,SHOWHGT);
    addGFXString(colorpair_gray_black,"E:" + sint2String(getPlayer()->getLevel()),45,SHOWHGT);
    addGFXString(colorpair_gray_black,"PACK:" + sint2String(getPlayer()->getInv()->getTotalWeight()) +
                 "/" + sint2String(getPlayer()->getMaxCarry()) + "    ",50,SHOWHGT);

    if (level_obj.getSpecLevVal() == -1)
    {
        addGFXString(colorpair_gray_black, "L:" + sint2String(level_number) + " ", 67, SHOWHGT);
    }
    else
    {
        addGFXString(colorpair_gray_black, "L:?? ", 67, SHOWHGT);
    }

    addGFXString(colorpair_gray_black,"T:" + sint2String(num_game_turns),72,SHOWHGT);
}
...
void Game::printCustomItemSymbols()
{
    for (int i = 0; i < level_obj.getNumItemPiles(); ++i)
    {
        chtype ct;
        point ip = level_obj.getPile(i)->getLoc();

        if (getMap()->getm(ip)) 
        {
            item_t itm = getMap()->getItem(ip);

            if ((getMap()->getNPC(ip) == NIL_n && getMap()->getFire(ip) == NIL_f)
                || !getMap()->getv(ip)) 
            {
                if (item_default_stats[(int)itm].category == ITEM_DEVICE)
                {
                    int index = deviceIdData.identificationMap[(int)itm - DEVICE_OFFSET];
                    disp.printCell(getMap(), ip, deviceSymbols[index]);
                }
            }
        }
    }
...
void Game::printCustomNPCSymbols()
{
    for (int i = 0; i < level_obj.getNumNPCs(); ++i)
    {
        chtype ct;
        point bp = level_obj.getBeing(i)->at();
        NPC_t nt = level_obj.getBeing(i)->getType();
...
void Game::pickUpItems(being *b, int s, int q)
{
    ...
    if (istats->category != ITEM_DEVICE) 
    {
        printPickupMessage(b, q, istats->name);
    }
    else 
    {
        int index = (int)istats->i_type - DEVICE_OFFSET;
        printPickupMessage(b, q, deviceIdData.discoveryMap[index] ? istats->name : unidentifiedDeviceNames[deviceIdData.identificationMap[index]]);
    }
...
// turned off for now
void Game::beingDropAllItems(being *b, point at, bool dropEverything)
{
    if (terrainAt(at) == WALL_DESTRUCT || terrainAt(at) == WALL_INDESTRUCT)
    {
        return;
    }
    ...
    if (b->getCredits() > 0)
    {
        level_obj.addCredits(b->getCredits(),at);
        b->setCredits(0);
    }
...
STRING damageStatusString(being *b)
{
    ...
    if (cur_hp == max_hp)
    {
        return beingStats(b)->isLifeform ? "Unharmed" : "undamaged";
    }
    
    if ((float)cur_hp >= 11 * (float)max_hp / 12)
    {
        return beingStats(b)->isLifeform ? "Barely scathed" : "Barely damaged";
    }

    if ((float)cur_hp >= 8 * (float)max_hp / 9)
    {
        return beingStats(b)->isLifeform ? "Somewhat hurt" : "Somewhat damaged";
    }

    if ((float)cur_hp >= 3 * (float)max_hp / 4)
    {
        return beingStats(b)->isLifeform ? "Lightly wounded" : "Lightly damaged";
    }

    if ((float)cur_hp >= (float)max_hp / 2)
    {
        return beingStats(b)->isLifeform ? "Wounded" : "Damaged";
    }
    
    if ((float)cur_hp >= (float)max_hp / 4)
    {
        return beingStats(b)->isLifeform ? "Highly wounded" : "Highly damaged";
    }

    if ((float)cur_hp >= (float)max_hp / 7)
    {
        return beingStats(b)->isLifeform ? "Severely wounded" : "Severely damaged";
    }

    return beingStats(b)->isLifeform ? "Dying" : "extremely damaged";
}

// util
STRING deathMessage(being *b)
{
    if (beingStats(b)->isLifeform)
    {
        switch (ROLL(7))
        {
            case(0):
...
{
    bool autoIdentifiedItems = false;

    // which item did the user select?
    int selection = 0;

    item_container * itc = NULL;
    ...
    do
    {
        printItemList(&deviceIdData, itc, selection, true, autoIdentifiedItems);
...
void Game::examineItemFromArmsDealer(int quant, int index, being *b)
{
    ...
    else
    {
        ...
        if (ibuf.checkForKey('p'))
        {
            ...
            if (i_stats->category == ITEM_DEVICE) 
            {
                deviceIdData.discoveryMap[(int)i_stats->i_type - DEVICE_OFFSET] = true;
            }
...
bool Game::checkNPCEquipEvent(being *b)
{
    ...
    if (PERCCHANCE() <= (uint)b->getStats()->equip_chance)
    {
        ...
        {
            NPCMessage(b,actionToMessage(b,"is searching " + getGenderPrefix(b->getGender()) + " pack for a weapon."),false);
        }
...
void Game::delayAndUpdateOnly(int ms)
{
    updateScreen();
    SDL_Delay(ms);
}
...
{
    if (it->getType() == NIL_i || (it->getStats()->category == ITEM_FIREARM_WEAPON && is_melee))
    {
        return 1;
    }
...
{
    msgeAdd("kaBOOOOOM!",colorpair_orange_black);
        ...
        for (int y = -radius; y <= radius; ++y)
        {
            ...
            for (int x = -x_range; x <= x_range; ++x)
            {
                ...
                if (inRange(dp, 0, 0, MAPWID - 1, MAPHGT - 1))
                {
                    ...
                    {
                        if ((x != 0 || y != 0) && isCellBlocking(getMap()->getNPC(dp)))
                        {
...
{
    if (creator->getID() != target->getID() && creator->getAllyValue() > 0 && target->getAllyValue() > 0)
    {
        return;
    }
...
being_inv * Game::playerInventory()
{
    return player.getInv();
}

item * Game::playerWeapon()
{
    return playerInventory()->getWeapon();
}

item * Game::playerArmor()
{
    return playerInventory()->getArmor();
}

terrain_t Game::terrainAt(point p)
{
    return getMap()->getTerrain(p);
}

item_t Game::itemAt(point p) 
{
    return getMap()->getItem(p);
}

NPC_t Game::NPCAt(point p) 
{
    return getMap()->getNPC(p);
}

fire_t Game::fireAt(point p) 
{
    return getMap()->getFire(p);
}

void Game::setTerrainAt(point p, terrain_t t)
{
    return getMap()->setTerrain(p, t);
}

void Game::setItemAt(point p, item_t i) 
{
    return getMap()->setItem(p, i);
}

void Game::setNPCAt(point p, NPC_t n) 
{
    return getMap()->setNPC(p, n);
}

void Game::setFireAt(point p, fire_t f) 
{
    return getMap()->setFire(p, f);
}

being* Game::getBeing(int i) 
{
    return level_obj.getBeing(i);
}

point Game::beingAt(int i) 
{
    return getBeing(i)->at();
}
...
double getArmorCost(item_stats *istats)
{
    ...
    double basepv_ratio = (double)(istats->armstats.basepv_value + 10.0) / (double)(item_default_stats[(int)istats->i_type].armstats.basepv_value + 1.0);
    double metallic_ratio = (double)(istats->armstats.metallic_bonus + 3.0) / (double)(item_default_stats[(int)istats->i_type].armstats.metallic_bonus + 1.0);
    double energy_ratio = (double)(istats->armstats.energy_bonus + 3.0) / (double)(item_default_stats[(int)istats->i_type].armstats.energy_bonus + 1.0);
...
double getWeaponCost(item_stats *istats)
{
    ...
    double damage_numdice_ratio = (double)istats->wepstats.dam.num_dice * 4/ ((double)item_default_stats[(int)istats->i_type].wepstats.dam.num_dice + 1.0);
    double damage_numsides_ratio = (double)istats->wepstats.dam.num_sides * 3 / ((double)item_default_stats[(int)istats->i_type].wepstats.dam.num_sides + 1.0);
    double damage_numbonus_ratio = (double)istats->wepstats.dam.num_bonus * 2/ ((double)item_default_stats[(int)istats->i_type].wepstats.dam.num_bonus + 1.0);
...
uint64 getItemCost(item_stats *istats, int quant, int level_number)
{
    ...
    switch(istats->category)
    {
        ...
        case(ITEM_AMMUNITION):
        {
            ret_val = istats->base_cost + ((level_number * level_number) - 1)*15;
            break;
        }
        case(ITEM_WEAPON_UPGRADE):
        {
            ...
            {
                ret_val = istats->base_cost * 10 * level_number;
            }

            if (istats->misc_val == 2)
            {
                ret_val = istats->base_cost * 100 * level_number;
            }
        ...
        if (ibuf.checkForKey('y'))
        {
            if (s->getCost() <= player_credits)
            {
                if (getMap()->getNPC(s->getSpawnLoc()) == NIL_n)
                {
                    ...
                    s->genSatelliteNPC(level_obj.getNPCIDCounter(), level_number);
...
void Game::checkForStunEvent(being *attacker, being *target, int dam, bool is_melee)
{
    ...
    if (attacker->getInv()->getWeapon()->getStats()->category == ITEM_FIREARM_WEAPON && is_melee)
    {
        return;
    }

    if (PERCCHANCE() > attacker->getInv()->getWeapon()->getStats()->wepstats.stun_chance || target->getStats()->is_stun_immune)
    {
        return;
    }
...
// util
STRING getGenderPrefix(being_gender gen)
{
    switch(gen)
    {
...
// utils
being_inv * inventory(being * b) 
{
    return b->getInv();
}

void clearInventory(being * b) 
{
    inventory(b)->deleteAllItems();
}

void setWeapon(being * b, const item_stats weaponStats)
{
    inventory(b)->setWeapon(weaponStats);
}

void setArmor(being* b, const item_stats armorStats) 
{
    inventory(b)->setArmor(armorStats);
}

void setWeapon(being* b, int i)
{
    inventory(b)->setWeapon(i);
}

void setArmor(being* b, int i)
{
    inventory(b)->setArmor(i);
}

void addToInventory(being* b, item_stats itemStats, int quantity) 
{
    inventory(b)->addToContainer(itemStats, quantity);
}

void incCredits(being* b, uint64 increment)
{
    uint64 baseQuantity = credits(b);
    b->setCredits(baseQuantity + increment);
}

void decCredits(being* b, uint64 decrement)
{
    uint64 baseQuantity = credits(b);
    b->setCredits(baseQuantity - decrement);
}

void setCredits(being* b, uint64 quantity) 
{
    b->setCredits(quantity);
}

uint64 credits(being * b)
{
    return b->getCredits();
}

being_stats * beingStats(being * b) 
{
    return b->getStats();
}

item_stats* itemStats(item* it) 
{
    return it->getStats();
}

point beingAt(being* b) 
{
    return b->at();
}
...
item * weapon(being * b) 
{
    return inventory(b)->getWeapon();
}

item* armor(being * b)
{
    return inventory(b)->getArmor();
}

dice_roll getDeathExplosionDamage(being * b)
{
    switch (b->getType())
    {
        case(ROBOT_NPC):
