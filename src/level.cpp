...
bool level::checkMiscForbiddenTileArea(point p)
{
    if (level_number == 1)
    {
       if (inRange(p, LEV1_ROOM_MINX, LEV1_ROOM_MINY,
                      LEV1_ROOM_MAXX, LEV1_ROOM_MAXY))
       {
          return true;
       }
    }
...
void level::addMiscFeatures(terrain_t t, int roller, bool f)
{
    ...
    } while (isAt(p,point(0,0)) || checkMiscForbiddenTileArea(p));
...
void level::addElevator(int lev)
{
    ...
    } while (isAt(loc,point(0,0)) || checkMiscForbiddenTileArea(loc));
...
void level::addItems()
{
    ...
    {
        if (!checkMiscForbiddenTileArea(point(x, y)))
        {
            if (getMap()->getTerrain(point(x, y)) == FLR_GEN)
            {
                if (ROLL(44 - level_number) == 0) 
                {
...
void level::addBeings()
{
    ...
    {
        rollOneNPC(ROBOT_NPC,HOPPER_NPC,NIL_n,true);
    }
...
void level::rollOneNPC(NPC_t start_npc, NPC_t end_npc, NPC_t specific_npc, bool initial_npc)
{
    ...
    {
        p = getRandWalkablePoint(1,1,MAPWID-2,MAPHGT-2);
    } while (m.getv(p) || isAt(p,point(0,0)) || (initial_npc && pointDist(p,getEntrance()) < 4) || checkMiscForbiddenTileArea(p));
...
bool level::addRandomNPC()
{
    ...
    rollOneNPC(ROBOT_NPC,HOPPER_NPC,NIL_n,false);

    return true;
}

void level::addCredits(uint64 creds, point at)
{
    addItem(item_default_stats[CREDIT],creds,at);
}
...
void level::addBeing(point p, int hp, being_stats bstat, bool agg, bool act)
{
    ...
    {
        beings[beings.size() - 1].setCredits(ROLL(9999 * level_number) + 1);
    }
    ...
    giveBeingWeaponsAmmoArmor(getBeing(beings.size()-1), level_number);
}
...
{ 
    ...
    item_category category = ITEM_NIL;
    
    do 
    {
        category = (item_category)(ROLL(NUM_ITEM_CATEGORIES - 2) + 1);
    } while (PERCCHANCE() > itemCategoryCreateChance[(int)category]);
    ...
    do
    {
        switch (category) 
        {
            case(ITEM_CREDIT):
            {
                addCredits(1 + (uint64)(ROLL((danger_factor * danger_factor) * 99)), p);
                return;
            }
            case(ITEM_AMMUNITION):
            {
                it = (item_t)rollrange((int)HANDGUN_AMMO_GEN, (int)PLASMA_AMMO);
                break;
            }
            case(ITEM_MELEE_WEAPON):
            {
                it = (item_t)rollrange((int)METAL_ROD, (int)SWORD_GIANT);
                break;
            }
            case(ITEM_FIREARM_WEAPON):
            {
                it = (item_t)rollrange((int)PISTOL_10MM, (int)PULSE_CANNON);
                break;
            }
            case(ITEM_WEAPON_UPGRADE):
            {
                it = (item_t)rollrange((int)PISTOL_10MM_UPGRADE, (int)PULSE_CANNON_UPGRADE);
                break;
            }
            case(ITEM_ARMOR):
            {
                it = (item_t)rollrange((int)GRAPHENE_ARMOR, (int)BALLISTIC_ARMOR);
                break;
            }
            case(ITEM_MEDKIT):
            {
                it = (item_t)rollrange((int)SMALL_MEDKIT, (int)GIANT_MEDKIT);
                break;
            }
            case(ITEM_STATBOOSTER):
            {
                it = (item_t)rollrange((int)STRENGTH_BOOSTER, (int)ACCURACY_BOOSTER);
                break;
            }
            case(ITEM_DEVICE):
            {
                it = (item_t)rollrange((int)DRILL_DEVICE, (int)CORRODING_DEVICE);
                break;
            }
            default:
            {
    ...
    while (PERCCHANCE() > (uint)item_default_stats[(int)it].frequency ||  danger_factor < item_default_stats[(int)it].min_level);
...
{
    const int danger_factor = level_number + 5;
...
void level::addSatellite(point p, int dl)
{
    ...
    satellites[satellites.size()-1].genSatelliteNPC(NPC_id_counter, level_number);
}
    ...
    for (int y = 0; y < dimy; ++y)
    {
        ...
        switch(room_vec[(dimx * y) + x])
        {
            ...
            case('G'):
            {
                ...
                elevators.push_back(elevator(p, 9 + ROLL(13)));
                break;
            }
            case('Y'):
            {
...
void level::initLevel1SpecialRoom()
{
    ...
    addDoor(point(27, 13), 0, "EXPRESS ELEVATOR", GENERIC_DOOR_TYPE);
}
