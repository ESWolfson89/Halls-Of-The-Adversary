...
void satellite::genSatelliteNPC(int NPC_id_counter, int currentLevel)
{
    ...
    giveBeingWeaponsAmmoArmor(&spawnable_being, currentLevel);
...
being_stats getProcgenStats(int d, int level_number)
{
    ...
    procgen_stats.spitAcidChance = (ROLL(25) == 0 ? ROLL(75) : 0);
...
void giveBeingWeaponsAmmoArmor(being *b, int currentLevel)
{
    ...
    for (int i = 0; i < NUM_TOTAL_WEAPONS; ++i)
    {
        uint carryChance = (uint)b->getStats()->carry_chance[i]; 
        
        if (PERCCHANCE() <= carryChance && currentLevel >= item_default_stats[i+WEAPON_OFFSET].min_level)
        {
    ...
    {
        uint wearChance = (uint)b->getStats()->wear_chance[i];
        
        if (PERCCHANCE() <= wearChance && currentLevel >= item_default_stats[i+ARMOR_OFFSET].min_level)
        {
